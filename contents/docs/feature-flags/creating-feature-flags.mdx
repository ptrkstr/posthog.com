...
Percentage rollouts are available for all flags. Rollout percentages support decimal values with up to two decimal places of precision (minimum 0.01%), which is useful for large-scale rollouts where you need to target less than 1% of users.

More options depend on your PostHog setup:
