...
| `sdk_doctor`             | [SDK health diagnostics](/docs/sdk-doctor)                                             |
...
**Prompt:** "List all cohorts in my project"

The agent will use the `cohorts-list` tool to return all cohorts, including both static cohorts (manually defined user lists) and dynamic cohorts (automatically updated based on property filters).

**Prompt:** "Create a dynamic cohort called 'power-users' for users who completed the purchase event more than 5 times in the last 30 days"

The agent will use the `cohorts-create` tool to create a dynamic cohort with the specified behavioral filter criteria.

**Prompt:** "Create a static cohort called 'beta-testers' and add specific users to it"

The agent will use the `cohorts-create` tool with `is_static: true` to create an empty static cohort, then use the `cohorts-add-persons-to-static-cohort-partial-update` tool to add person UUIDs to it.

### Path analysis

**Prompt:** "What are the most common paths users take after signing up?"

The agent will use the `query-paths` tool to analyze user journeys, returning the flow of events users follow after signup as a directed graph.

**Prompt:** "Show me the navigation paths from the pricing page to checkout"

The agent will run a paths query with `startPoint` set to the pricing page and `endPoint` set to checkout, revealing the most common routes users take and where they drop off.

### Funnel analysis

**Prompt:** "What's the conversion rate from signup to first purchase?"

The agent will use the `query-funnel` tool to run a funnel query with signup and purchase events as steps, returning conversion rates and drop-off points.

**Prompt:** "Show me how users convert through our onboarding flow: signup, profile setup, first project created"

The agent will run a multi-step funnel query analyzing conversion rates between each step and where users drop off.

### Retention analysis

**Prompt:** "What percentage of users who signed up last month came back and used the app in week 2?"

The agent will use the `query-retention` tool to analyze user return rates, showing a cohort-based retention matrix.

**Prompt:** "Show me retention for users who completed onboarding – are they sticking around?"

The agent will run a retention query filtered to users who completed onboarding, returning weekly retention percentages.

### LLM trace inspection

**Prompt:** "Show me the slowest LLM traces from the past hour"

The agent will use the `query-traces-list` tool to fetch LLM traces sorted by latency, returning details including model, token usage, and response times.
...
The agent will use the `conversations-tickets-list` tool with a status filter to return all open tickets, including ticket numbers, priorities, message counts, and timestamps.

**Prompt:** "Get details for support ticket #5"

The agent will use the `conversations-tickets-retrieve` tool to fetch the full ticket details by ticket number, including status, priority, channel info, message history, and session context.

**Prompt:** "Mark ticket #3 as resolved and set ticket #7 to high priority"

The agent will use the `conversations-tickets-update` tool to change the status and priority of the specified tickets.
